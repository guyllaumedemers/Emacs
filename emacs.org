...
#+begin_src emacs-lisp
  ...
  (defun cmake-make-shell-config ()
    (interactive)
    (if (eq system-type 'windows-nt)
        (shell-command (format "echo cmake -S . -B out\\build -G %s -D CMAKE_BUILD_TYPE=DEBUG >> cmake-config.cmd" build_system))))
...
CMake can be run with a multitude of options that allow you to peek under the hood. To get general information about variables, commands, macroa, and other settings.

#+begin_src emacs-lisp

  ;; M-x run-cmake-debugtrack to execute the .cmd file created below
  (defun run-cmake-debug-trace (&optional file)
    (interactive "sOutput filename: ")
    (if (eq system-type 'windows-nt)
