...
A set of Scripts to run in Shell.

*** Create Empty Shell Script 

#+begin_src emacs-lisp

  (defun init-empty-shell-script (script)
    (interactive "sScript name: ")
    (when (string-match (format "which: no %s" script) (shell-command-to-string (format "which %s" script)))
      (shell-command (format "echo \\#\\!%S > \~/bin/%s" shell-file-name script))))
...
#+begin_src emacs-lisp
  ...
  ;; makes sure the upgrade process doesnt remove your install configuration
  (defun choco-enable-remember-args()
    (interactive)
    (shell-command "choco feature enable -n useRememberedArgumentsForUpgrades"))

  ;; double check the feature list to confirm the above command was set
  (defun choco-feature-list ()
    (interactive)
    (shell-command "choco feature list"))
...
#+begin_src emacs-lisp
  ...
  (defun install-git ()
    (interactive)
    (shell-command "choco install git --params \"'/GitAndUnixToolsOnPath'\" -y"))
...
#+begin_src emacs-lisp
  ...
  (defun install-cppcheck ()
    (interactive)
    (shell-command "choco install cppcheck -y"))
...
#+begin_src emacs-lisp
  ...
  (defun init-gitignore ()
    (interactive)
    (get_https_request "https://raw.githubusercontent.com/guyllaumedemers/emacs-get-request/master/.gitignore" ".gitignore"))
...
#+begin_src emacs-lisp
  ...
  ;; put under root to pre-configure clang with target
  (defun cmake-init-root-toolchain ()
    (interactive)
    (get_https_request "https://raw.githubusercontent.com/guyllaumedemers/emacs-get-request/master/toolchain.cmake" "toolchain.cmake"))
...
#+begin_src emacs-lisp
  ...
  ;; cmake flags
  (setq build_path "\'out\\build\'")
  (setq build_system "\'Unix Makefiles\'")
  (setq build_toolchain "\'.\\toolchain.cmake\'")
  (setq compilation_mode "\'DEBUG\'")
  (setq compile_commands "\'ON\'")

  (defun init-cmake-config-toolchain ()
    (interactive)
    ;; note: string output in .sh later invoke in bash shell so \ behaviour need to be handled twice
    (shell-command (format "echo cmake -S . -B %S -G %S --toolchain %S -DCMAKE_BUILD_TYPE=%S -DCMAKE_EXPORT_COMPILE_COMMANDS=%S > cmake-config.sh" build_path build_system build_toolchain compilation_mode compile_commands)))
...
****** Create CMake-Build.sh

#+begin_src emacs-lisp

  (defun init-cmake-build-toolchain ()
    (interactive)
    (shell-command (format "echo cmake --build %S > cmake-build.sh" build_path)))
...
#+begin_src emacs-lisp
  ...
  (defun run-cmake-debug-trace (file)
...
#+begin_src emacs-lisp
  ...
  (defun init-project-root-dir (project)
...
#+begin_src emacs-lisp
  ...
  (defun cmake-init-git-module ()
    (interactive)
    (get_https_request "https://raw.githubusercontent.com/guyllaumedemers/emacs-get-request/master/git.cmake" "git.cmake"))

#+end_src

***** CMakeLists.txt

A set of shell templates for generating listfile.
...
Other Ide services used for running c++ projects or cmake-based projects.

** Rider

Tips: Usefull when doing game development like Unreal Engine. Doesn't support cmake-based projects and require a project solution.

#+begin_src emacs-lisp

  (defun install-rider()
    (interactive)
    (shell-command "choco install jetbrains-rider -y"))

#+end_src

** CLion

Tips: Usefull when running cmake-based projects.

#+begin_src emacs-lisp

  (defun install-clion()
    (interactive)
    (shell-command "choco install clion-ide -y")) 
...
*NOTE:* The first time you load your configuration on a new machine, you'll need to run M-x all-the-icons-install-fonts` so that mode line icons display correctly. Also run M-x nerd-icons-install-fonts if emacs modeline icons are not displaying. Make sure downloaded fonts are installed on your Operating system and reload emacs.
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'org (when (require 'org-tempo nil 'noerror)
...
A set of tools to embeed standard ide feature.
...
A set of tools to be run to simulate an ide in emacs.
...
#+begin_src emacs-lisp
  ...
  (defun efs/lsp-clients-exec-setup ()
    (setq lsp-clients-clangd-executable (car (get_file_using_regex "LLVM" "clangd.exe"))))

  (defun efs/lsp-clients-clangd-args-setup ()
    (let (result)
      (add-to-list 'result "--header-insertion=iwyu")
      (add-to-list 'result "--completion-style=bundled")
      (add-to-list 'result "--log=info")
      (add-to-list 'result "--pretty")
      (setq lsp-clients-clangd-args result)))

  (defun efs/lsp-clients-clangd-include-directories-setup ()
    (add-to-list 'lsp-clients-clangd-library-directories (car (last (get_directories_using_regex "mingw64")))))

  (use-package lsp-mode
    :commands
    (lsp lsp-deferred)
    :init
    (efs/lsp-clients-exec-setup)
    (efs/lsp-clients-clangd-args-setup)
    ...
    (lsp-mode . efs/lsp-clients-clangd-include-directories-setup)
...
#+begin_src emacs-lisp
  ...
  (defun efs/company-backends-config ()
...
A set of tools related to compilers.
...
#+begin_src emacs-lisp
  ...
  (add-to-list 'load-path (format "%s\\%s" (replace-regexp-in-string "bin" "share" (car (get_directories_using_regex "LLVM"))) "clang"))
...
https://clangd.llvm.org/config.html https://clangd.llvm.org/design/indexing  

#+begin_src emacs-lisp

  (defun init-clangd ()
    (interactive)
    (get_https_request "https://raw.githubusercontent.com/guyllaumedemers/emacs-get-request/master/.clangd" ".clangd"))
...
#+begin_src emacs-lisp
  ...
  ;;   :hook
  ;;   (lsp-mode . highlight-doxygen-mode)
  ;;   (c++-mode . highlight-doxygen-global-mode)
  ;;   (c-mode . highlight-doxygen-global-mode))
