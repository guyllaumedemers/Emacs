...
#+begin_src emacs-lisp
  ...
  (defun set-system-shell (exec)
    (interactive "sExecutable name: ")
    (when (eq system-type 'windows-nt)
      (let ((shell (get_executable_using_regex "Git" "bin")))
...
A shorten version of a generic regex entry.
...
*** get_executable_using_regex

A macro to retrieve path for a specific target.

#+begin_src emacs-lisp

  (defun get_executable_using_regex (folder exec)
    (let (result)
      (dolist (directory (get_system_path) result)
        (if (string-match (format "%s*%s%s*%s" (get_regex_wildcard) folder (get_regex_wildcard) exec) directory)
...
#+begin_src emacs-lisp
  ...
  ;; M-x init-project-root-dir - create the root directory tree
...
#+begin_src emacs-lisp
  ...
  ;; M-x init-project-src-application-dir - create the application directory tree
...
#+begin_src emacs-lisp
  ...
  ;; Internal use ONLY - create top-level CMakeLists.txt
...
#+begin_src emacs-lisp
  ...
  ;; Internal use ONLY - create src-level CMakeLists.txt
...
#+begin_src emacs-lisp
  ...
  ;; Internal use ONLY - create app-level CMakeLists.txt
