...
#+begin_src emacs-lisp
  ...
  (defun get_regex_wildcard ()
    (let ((result "[a-zA-Z0-9\\\/\.\:\+\_\-\~]"))
      result))
...
*** get_directory_using_regex

A macro to retrieve path for a specific target.

#+begin_src emacs-lisp

  (defun get_directory_using_regex (target exec)
    (let (result)
      (dolist (directory (get_system_path) result)
        (if (string-match (format "%s*%s%s*%s" (get_regex_wildcard) target (get_regex_wildcard) exec) directory)
            (setq result directory)))
      result))
...
A macro to retrive the file at path.

#+begin_src emacs-lisp

  (defun get_file_using_regex (target file)
    (let (result)
      (dolist (elem (directory-files target))
        (if (string-match (format "%s" file) elem)
            (setq result (format "%s\\%s" target elem))))
      result))
...
#+begin_src emacs-lisp
  ...
  (defun set-unix-system-shell (exec)
    ...
    (when (eq system-type 'windows-nt)
      (let ((shell (get_directory_using_regex "Git" "bin")))
        (setq shell-file-name (format "%s\\%s.exe" shell exec))
        (shell-command (format "setx SHELL \'%s\\%s.exe\'" shell exec)))))
...
#+begin_src emacs-lisp
  ...
  (defun init-gitignore ()
      ...
      (with-temp-file "\.gitignore"
        (insert "# Created by https://www.toptal.com/developers/gitignore/api/cmake,c++,c
  # Edit at https://www.toptal.com/developers/gitignore?templates=cmake,c++,c
  ...
  # End of https://www.toptal.com/developers/gitignore/api/cmake,c++,c

  ### wildcard removing trash files created by vim, notepad++
  ,*.bak
  ,*.un~
  ,*.txt~
  ,*~
  ...
  ### remove visual studio chached dir
  .vs")))
...
#+begin_src emacs-lisp
  ...
  (defun cmake-init-shell-config ()
    ...
    (shell-command (format "echo cmake -S . -B \"\'out\\build\'\" -G \"%s\" -D CMAKE_BUILD_TYPE=\"%s\" -D CMAKE_C_COMPILER=\"%s\" -D CMAKE_CXX_COMPILER=\"%s\" -D CMAKE_EXPORT_COMPILE_COMMANDS=\'1\' -D CMAKE_CXX_EXTENSIONS=\'OFF\' > cmake-config.sh" build_system compilation_mode c_compiler c++_compiler)))
...
#+begin_src emacs-lisp
  ...
  (defun cmake-init-src-cmakelist (&optional app)
...
#+begin_src emacs-lisp
  ...
  (defun efs/lsp-clients-exe-setup ()
    (setq lsp-clients-clangd-executable (get_file_using_regex (get_directory_using_regex "LLVM" "bin") "clangd.exe")))

  (defun efs/lsp-clients-lib-setup ()
    (let ((elem (get_directory_using_regex "mingw" "gcc")))
      (add-to-list 'lsp-clients-clangd-library-directories elem)))
...
#+begin_src emacs-lisp
  ...
  (defun find_debugger_path (&optional debugger)
    (setq system_path (cl-remove-duplicates (split-string (getenv "PATH") ";") :test 'string=))
    (dolist (elem system_path)
      (when (string-match "[a-zA-Z0-9\\\/\.\:\_\+\-]*LLVM\\bin" elem)
        return elem)))

  (defun init-dap-json (&optional app &optional debugger)
    (interactive "sApplication name: \nsDebugger name: ")
      ...
      \"configurations\": [
          {
              ...
              \"type\": \"cppdbg\",
              \"request\": \"launch\",
              \"program\": \"${workspaceFolder}/out/build/bin/%s\",
              \"args\": [\"-q\"],
              \"stopAtEntry\": false,
              \"cwd\": \"${workspaceFolder}\",
              \"environment\": [],
  ...
  }" app debugger (find_debugger_path debugger)))))
