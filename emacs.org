...
#+begin_src emacs-lisp
  ...
  (defun cmake-init-shell-config ()
    ...
    (shell-command (format "echo cmake -S . -B \"\'out\\build\'\" -G \"%s\" -D CMAKE_BUILD_TYPE=\"%s\" -D CMAKE_C_COMPILER=\"%s\" -D CMAKE_CXX_COMPILER=\"%s\" -D CMAKE_EXPORT_COMPILE_COMMANDS=1 -P \"\'cmake\\git.cmake\'\" > cmake-config.sh" build_system compilation_mode c_compiler c++_compiler)))
