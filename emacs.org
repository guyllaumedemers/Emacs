...
Run the following command at the application level to handle include directories in emacs.

#+begin_src emacs-lisp

  (defun recursive-search-includes ()
    ;; cd src
    ;; get all directories at point
    ;; search for includes folder
    ;; recurse until no match
    ;; run regex on each directory path returned
    ;; prepend -I and wrap the path in \" \" to properly append the list below
    )

  (defun set-project-include-directories (directories)
    (let (result)
      (setq result "((nil . ((company-clang-arguments . (%s)))))")
      (setq result (format result directories))
      result))

  (defun init-dir-locals ()
    (interactive)
    (shell-command (format "echo \'%s\' > .dir-locals.el" (set-project-include-directories (recursive-search-includes)))))
