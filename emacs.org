...
#+begin_src emacs-lisp
  ...
  ;; Call this first when starting a new project

  ;; M-x init-project-root-dir - create the root directory tree
  (defun init-project-root-dir (&optional project)
    (interactive "sProject name: ")
    (shell-command (format "mkdir -p %s" project))
    (shell-command (format "cd %s && mkdir -p {cmake,doc,extern,out,res,src,test}" project))
...
#+begin_src emacs-lisp
  ...
  (defun cmake-init-root-cmakelist (&optional project)
    ...
    (dolist (elem (list "doc" "extern" "test"))
      (with-temp-file (format "%s\\CMakeLists.txt" elem)
        (insert (format "")))))
...
#+begin_src emacs-lisp
  ...
  (defun cmake-init-app-cmakelist (&optional app)
    (with-temp-file (format "%s\\CMakeLists.txt" app)
      (insert (format "cmake_minimum_required(VERSION %s)\nproject(%s VERSION 0.0.1)\n\nfile(GLOB_RECURSE \"${PROJECT_NAME}_SRC\" \"${CMAKE_CURRENT_SOURCE_DIR}\" \"*.cpp\")\n\nadd_library(\"${PROJECT_NAME}\" OBJECT \"${PROJECT_NAME}_SRC\")\ntarget_include_directories(\"${PROJECT_NAME}\" PUBLIC \"${CMAKE_CURRENT_SOURCE_DIR}/includes\")" cmake_version app))))
...
#+begin_src emacs-lisp
  ...
  (defun init-main-cc (&optional app)
    (with-temp-file (format "%s\\Main.cc" app)
...
#+begin_src emacs-lisp
  ...
  ;; (defun init-project (&optional project &optional app)
  ;;   (interactive "sProject name: %s \n sApplication name: %s")
  ;;   (init-project-root-dir project)
  ;;   (init-project-src-application-dir app))
